(function () {
  ...
  syncHoyPromoMarquee();
  document.addEventListener('DOMContentLoaded', () => syncHoyPromoMarquee());
  ...
  function syncHoyPromoMarquee(attempt = 0) {
    const promoGroups = document.querySelectorAll('.hoy-header-marquee--promo .hoy-header-marquee__group');

    if (!promoGroups.length) {
      if (attempt < 30) setTimeout(() => syncHoyPromoMarquee(attempt + 1), 100);
      return;
    }
    ...
    promoGroups.forEach((group) => {
      ...
      group.innerHTML = new Array(6).fill(newArrivalLink).join('');
      group.dataset.hoyPromoUpdated = 'true';
    });
  }
})();
